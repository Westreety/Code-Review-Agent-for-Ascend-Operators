...
基于 `ops-math/math/mul/` 源码的完整分析，将 Mul 算子可能出现的错误划分为 **9 大类 43 种**，覆盖 op_api → op_host → op_kernel 三层架构及分布式场景。
...
> - 存档：`ERROR_TAXONOMY_v1.0.md`（v1.0 原文）
...
八类：非确定性计算错误 / Heisenbug（7种）
  ├── 8.1 同步屏障缺失（含异步 DMA 数据竞争）
  ├── 8.2 脏内存/未初始化内存
